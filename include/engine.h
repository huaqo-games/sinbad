...
typedef enum { NOT_CLICKED, HOVER, CLICKED } ButtonState;
...
void UpdateCameraZoom(const float *scaleFactor, const float *minZoom,
                      const float *maxZoom);
...
const char *GetConfigStringOrDefault(Config *config, const char *key,
...
int SetConfigString(Config *config, const char *key, const char *text,
                    const char *desc);
...
ImageButton CreateImageButton(ButtonState state, Texture2D *texture,
                              const TextureAsset *asset, Vector2 *source,
                              Vector2 *clicked, Vector2 *hover,
                              Rectangle *destRec);
ImageElement CreateImageElement(Texture2D *texture, const TextureAsset *asset,
                                Vector2 *source, Rectangle *destRec);
...
void AddObjectToArray(ObjectArray *objects, const Object *prototype,
                      Vector2 position);
void SetPositionForObjectArray(ObjectArray *objects,
                               const Vector2Array *positions);
void SetRandomPositionForObjectArray(ObjectArray *objects,
                                     const Rectangle bounds);
void SwapAndPopObjectsArrayOnRecCollision(ObjectArray *objects,
                                          const Rectangle rec);
void SwapAndPopObjectsArrayOnPointCollision(ObjectArray *objects,
                                            const Vector2 point);
