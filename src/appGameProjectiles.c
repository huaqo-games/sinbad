...
typedef enum { CANNONBALL_SHOOTING, CANNONBALL_IMPACT, SOUND_COUNT } SoundID;

const SoundAsset soundAssets[SOUND_COUNT] = {{"assets/cannonball_shooting.mp3"},
...
Projectiles CreateProjectiles(void) {
  ...
  for (int i = 0; i < projectiles.count; i++) {
    ...
    projectiles.soundShooting[i] =
        LoadSound(soundAssets[CANNONBALL_SHOOTING].path);
...
void UpdateProjectiles(Projectiles *projectiles, Enemies *enemies) {
  ...
  for (int i = 0; i < projectiles->count; i++) {
    ...
    UpdatePhysics(&projectiles->physics[i], projectiles->physics[i].direction);
  }
}
